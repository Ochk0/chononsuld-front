...
export default function SidebarRight() {
    ...
    function handleClose(data:any) {
        setFilter((el: any) => el.filter((datum: any) => datum.value != data.value))
    ...
    <div className="flex grow flex-col gap-y-5 h-full overflow-y-auto bg-[#154185] px-6 text-indigo-200">
      ...
      <nav className="flex  flex-1 flex-col">
          <form className='space-y-4'>
              {filter?.map((filters:any) => <DropDownBox key={filters.value} handleClose={(data:any) => handleClose(data)} data={filters} />)}
              <ul className='flex flex-col space-y-2 items-center justify-center'>
                  ...
                  <DropdownMenu>
                      ...
                      <DropdownMenuContent className='bg-white cursor-pointer w-full text-[#154185] rounded-md focus:outline-none p-1' >
                          {data?.map((datas:any) => <DropdownMenuItem onClick={() => addFilter(datas)} className='p-2 hover:bg-[#154185]/20 rounded-md' key={datas.value}>{datas.label}</DropdownMenuItem>)}
                      </DropdownMenuContent>
                  </DropdownMenu>
              </ul>
          </form>
